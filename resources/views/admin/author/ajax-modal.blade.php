<!-- Modal -->
<form id="formWrapperModalAuthor" method="POST" action="{{ route('admin.author.store') }}" class="needs-validation" novalidate>
  <div class="modal fade" id="popupModal" tabindex="-1" role="dialog" aria-labelledby="authorModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="authorModalLabel">Data Pengarang</h5>
...
$(function() {
  ...
  popupDatatables
    .on( 'select', function ( e, dt, type, indexes ) {
      ...
      console.log(rowData );
      $('[name="author_id"]').val(rowData['id']);
      $('[name="author"]').val(rowData['name']);
  ...
  $("#formWrapperModalAuthor").submit(function(e){
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function(data, textStatus, jqXHR) {
          if (data['status'] == true) {
            $('[name="author_id"]').val(data['data']['id']);
            $('[name="author"]').val(data['data']['name']);
            $('#popupModal').modal('hide');
          }
          else {
            printErrorMsg(data.errors, '#formWrapperModalAuthor');
          }
